...
def multidim_search(xspace,
    ...
    while (vol_border >= delta) and (step <= max_step):
        ...
        ################################
        # Optimisations:
        # Every Incomparable rectangle that dominates B0 (Ylow) is included in Ylow
        #ylow_candidates = [inc for inc in i if any(less_equal(inc.max_corner, b0.min_corner) for b0 in ylow)]
        ylow_candidates = [inc for inc in i if any(inc.dominatesRect(b0) for b0 in ylow)]
        ylow.extend(ylow_candidates)
        len_ylow_candidates_2 = len(ylow_candidates)  ##
        vol_ylow_opt_2 = sum(b0.volume() for b0 in ylow_candidates)  ##
        vol_ylow += vol_ylow_opt_2
        for rect in ylow_candidates:
            i.remove(rect)

        # Every Incomparable rectangle that is dominated by B1 (Yup) is included in Yup
        #yup_candidates = [inc for inc in i if any(less_equal(inc.max_corner, b1.min_corner) for b1 in yup) ]
        #yup_candidates = [inc for inc in i if any(less_equal(b1.max_corner, inc.min_corner) for b1 in yup)]
        yup_candidates = [inc for inc in i if any(inc.isDominatedByRect(b1) for b1 in yup)]
        yup.extend(yup_candidates)
        len_yup_candidates_2 = len(yup_candidates) ##
